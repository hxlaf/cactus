...
export const IROHA_TEST_LEDGER_DEFAULT_OPTIONS = Object.freeze({
  ...
  containerImageName: "hyperledger/cactus-iroha-all-in-one",
  rpcDebuggerPort: 20000,
  rpcTorriPort: 50051,
  rpcTLSPort: 55552,
...
export const IROHA_TEST_LEDGER_OPTIONS_JOI_SCHEMA: Joi.Schema = Joi.object().keys(
  {
    ...
    rpcDebuggerPort: Joi.number().min(1024).max(65535).required(),
    rpcTorriPort: Joi.number().min(1024).max(65535).required(),
    rpcTLSPort: Joi.number().min(1024).max(65535).required(),
...
export class IrohaTestLedger implements ITestLedger {
  ...
  public async getRpcDebuggerPortHost(): Promise<string> {
    const ipAddress = "127.0.0.1";
    const hostPort: number = await this.getRpcDebuggerPort();
  ...
  public async getRpcTLSPortHost(): Promise<string> {
    const ipAddress = "127.0.0.1";
    const hostPort: number = await this.getRpcTLSPort();
    return `http://${ipAddress}:${hostPort}`;
  }
